...
echo "==> Creating dedicated snapshots subvolume..."
sudo btrfs subvolume delete /.snapshots 2>/dev/null || true

run_step \
  "Mounting Btrfs root volume" \
...
if ! grep -q "@snapshots" /etc/fstab; then
  echo "UUID=$UUID /.snapshots btrfs subvol=@snapshots,compress=zstd,noatime 0 0" | sudo tee -a /etc/fstab
fi
...
echo "==> Permissions..."
run_step \
  "Setting snapshot permissions" \
  sudo chmod 750 /.snapshots

echo "==> Enabling automatic timeline snapshots..."
run_step \
  "Enabling snapper timeline" \
  sudo systemctl enable --now snapper-timeline.timer

run_step \
  "Enabling snapper cleanup" \
  sudo systemctl enable --now snapper-cleanup.timer

echo "==> Enabling grub-btrfs daemon..."
run_step \
  "Enabling grub-btrfs daemon" \
  sudo systemctl enable --now grub-btrfsd

echo "==> Rebuilding GRUB..."
run_step \
  "Rebuilding GRUB after snapshots setup" \
  sudo grub-mkconfig -o /boot/grub/grub.cfg

echo "==> Creating initial snapshot..."
run_step \
  "Creating initial snapshot" \
  sudo snapper -c root create -d "Fresh Arch baseline"
...
run_step \
  ...
  git clone https://github.com/microsoft/BitNet.git "$REPO"
